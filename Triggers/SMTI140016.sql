...
PRINT ' '
PRINT ' -- **************************************'
PRINT ' -- **            SMTI140016.sql ' 
PRINT ' -- **************************************'


 -- Atualiza tabela de controle de scripts executados
insert into gemco_scripts_executados (script, dthrexecucao) select 'SMTI140016.sql', getdate()
go

IF EXISTS (SELECT NAME FROM SYSOBJECTS WHERE NAME = 'NF_SAIDA_TR_INS' AND TYPE='TR')
...
--                 END
  ...
  codoper,
       ...
       BEGIN --Roberto Cesar - 135549
		...
		@FLCONTINGENCIA, @TPAMBNFE, @FLNOTAMANUALPDV, @FLOFFLINE 
