...
PRINT ' '
PRINT ' -- **************************************'
PRINT ' -- **            SMTI150015.sql ' 
PRINT ' -- **************************************'


 -- Atualiza tabela de controle de scripts executados
insert into gemco_scripts_executados (script, dthrexecucao) select 'SMTI150015.sql', getdate()
go
...
declare SMART cursor static for    
    select TPNOTA,NUMNOTA,DTNOTA,SERIE,CODPROD,ITEM,QTCOMP,PRECOUNIT,ALIQICM,    
           ...
           VLIOFSEG, ITPEDITEM, CODAUTDESCPDV, ALIQIBPTEST, ALIQIBPTMUN, 
           CHAVEIBPT, FONTEIBPT, VLIBPTEST, VLIBPTMUN
      from INSERTED    
...
fetch SMART into @STPNOTA,@SNUMNOTA,@SDTNOTA,@SSERIE,@SCODPROD,@SITEM,@SQTCOMP,@SPRECOUNIT,@SALIQICM,    
                 ...
                 @ALIQIBPTEST, @ALIQIBPTMUN, @CHAVEIBPT, @FONTEIBPT, @VLIBPTEST, @VLIBPTMUN
